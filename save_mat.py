...
def Save_mat(epoch , output_dim , datasets , query_labels , retrieval_labels , query_img , retrieval_img , save_dir='..' , mode_name="DSH"):
    '''
    save_dir: 保存文件的目录路径
    output_dim: 输出维度
    datasets: 数据集名称
    ...
    mode_name: 模型的名称
    '''
    save_dir = os.path.join(save_dir , "Hash_code_and_label")
    os.makedirs(save_dir,exist_ok=True)
    ...
    filename = os.path.join(save_dir, f"{output_dim}-{epoch}-{datasets}-{mode_name}.mat")
